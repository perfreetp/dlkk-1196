...
const typeConfig: Record<Notice['type'], { icon: typeof Bell; label: string; color: string }> = {
  customer_service: {
    icon: Users,
    label: '客服通知',
    ...
    >
      <div className="flex items-start justify-between mb-3">
        ...
        <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${status.color}`}>
          <StatusIcon className={`w-3 h-3 ${notice.status === 'scheduled' ? '' : ''}`} />
          {status.label}
        </div>
      </div>
        ...
        <div className="mt-3 pt-3 border-t border-slate-700/30">
          <div className="flex items-center justify-between text-xs mb-1">
            ...
            <span className="text-slate-300">
              {notice.sentCount} / {notice.targetCount}
            </span>
          </div>
          <div className="h-1 bg-slate-700/50 rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-emerald-500 to-teal-400 rounded-full transition-all duration-700"
              style={{ width: `${(notice.sentCount / notice.targetCount) * 100}%` }}
            />
          </div>
        </div>
      )}
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto">
        <div>
          ...
          <input
            ...
            className="w-full bg-slate-900/60 border border-slate-700/50 text-white text-sm rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500/50 placeholder:text-slate-500"
            placeholder="请输入公告标题"
          />
        </div>
        ...
        <div>
          ...
          <div className="border border-slate-700/50 rounded-lg overflow-hidden">
            ...
            <textarea
              ...
              rows={12}
              className="w-full bg-slate-900/40 text-slate-200 text-sm px-4 py-3 focus:outline-none resize-none"
              placeholder="请输入公告内容..."
            />
          </div>
        </div>
      ...
      <div className="pt-4 border-t border-slate-700/50 mt-4 flex gap-3">
        <button className="flex-1 py-2.5 px-4 bg-slate-700/50 hover:bg-slate-700 text-slate-200 text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2">
        ...
        {notice.status === 'scheduled' && (
          <button className="flex-1 py-2.5 px-4 bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20">
          ...
          <div className="p-4 border-b border-slate-700/50 space-y-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="搜索公告..."
                className="w-full bg-slate-900/60 border border-slate-700/50 text-slate-200 text-sm rounded-lg pl-9 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500/50 placeholder:text-slate-500"
              />
            </div>
