      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div
          className={`w-2 h-2 rounded-full ${
            isNormal ? 'bg-emerald-400' : isWarning ? 'bg-amber-400' : 'bg-red-400'
        ...
        <div>
          <span
            className={`text-2xl font-bold ${
              isNormal ? 'text-white' : isWarning ? 'text-amber-400' : 'text-red-400'
            }`}
          >
            {metric.value}
          </span>
        ...
        <div className="h-1.5 bg-slate-700/50 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-700 ${
              isNormal ? 'bg-emerald-500' : isWarning ? 'bg-amber-500' : 'bg-red-500'
            }`}
...
function StrategyCard({ strategy }: { strategy: CanaryStrategy }) {
  const { pauseCanary, resumeCanary, updateCanaryPercent } = useAppStore();
  ...
  const phaseProgress = (strategy.currentPhase / strategy.totalPhases) * 100;
    ...
    <div className="bg-slate-800/40 backdrop-blur border border-slate-700/50 rounded-2xl overflow-hidden">
      ...
      <div className="p-6 space-y-6">
        <div>
          ...
          <input
            ...
            disabled={!isRunning}
            style={{
              background: `linear-gradient(to right, #3B82F6 0%, #3B82F6 ${strategy.userPercent}%, #334155 ${strategy.userPercent}%, #334155 100%)`,
            }}
          />
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {strategy.whitelist.map((account) => (
              <span
                key={account}
                className="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-500/10 text-emerald-400 text-xs rounded-md border border-emerald-500/20"
              >
                {account}
        ...
        <div className="flex items-center gap-3 pt-2">
          <div className="flex items-center gap-2 text-slate-500 text-sm">
            ...
            <span>开始时间: {strategy.startTime}</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-4 border-t border-slate-700/50 flex gap-3">
        {isRunning && (
          <>
            <button
              onClick={() => pauseCanary(strategy.id)}
              className="flex-1 py-2.5 px-4 bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
            >
              <Pause className="w-4 h-4" />
              暂停灰度
            </button>
            <button className="flex-1 py-2.5 px-4 bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2 shadow-lg shadow-blue-500/20">
        ...
        {isPaused && (
          <>
            <button
              onClick={() => resumeCanary(strategy.id)}
              className="flex-1 py-2.5 px-4 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
            >
              <Play className="w-4 h-4" />
              继续灰度
            </button>
            <button className="flex-1 py-2.5 px-4 bg-red-500/20 hover:bg-red-500/30 text-red-400 text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2">
              终止并回滚
            </button>
          </>
        )}
